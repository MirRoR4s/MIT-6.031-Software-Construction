...
1. Create an empty local directory `ps0`, and `ps0/.git`.
   ...
   ![image-20231222093750408](images/image-20231222093750408.png)

2. Connect to github.mit.edu and copy the object graph from `ps0-bitdiddle.git` into `ps0/.git`.

   ![image-20231222093819383](images/image-20231222093819383.png)
...
> See them with e.g. `git log`.

How many different versions of `hello.txt` are there?

> See only the commits that modified that file with e.g. `git log hello.txt`.

How many new files have been added to the project?
...
> `git log --name-status` can answer these last three questions. `A` means a file was added, `M` means modified. `D` is deleted.
