...
以下是用于计算和打印从 n 开始的冰雹序列的 Python 和 Java 代码。
...
在上述 Java 和 Python 代码之间最重要的语义差别就是变量 n 的声明，Java 在声明该变量时指定了其类型：`int`。

<!-- A **type** is a set of values, along with operations that can be performed on those values. -->

类型是一些值以及可对这些值进行的操作组成的集合。
...
根据 Java 的惯例，基本数据类型是小写的，而对象类型的首字母则是大写的。
...
**操作**实际上就是函数——接收输入然后产生输出（有时也会改变输入本身）。同一操作可能有不同的语法形式，但无论它们是如何书写的，我们仍可将它们视为函数。以下是 Python 或 Java 中的某操作的三种不同语法形式：
...
某些操作可以被重载。在这种情况下，不同的类型使用了相同的操作名。在 Java 中，用于基本的数字类型的算术运算符如 +，-，*，/ 被大量重载。方法也可以被重载。大部分的编程语言都有不同程序的重载。
...
Java 是一种静态类型的语言，所有变量的类型在编译时（在程序运行之前）就已知晓，所以编译器可以推断所有的表达式的类型。如果 a、b 都被声明为 int，那么编译器就推断出 a+b 也是 int。

<!-- In **dynamically-typed languages** like Python, this kind of checking is deferred until runtime (while the program is running). -->
在像 python 这样的动态类型语言中，这类检查被推迟到运行时进行（即程序运行的时候）。
...
静态类型是一类特殊的静态检查，会在编译时就检查 bug 的存在。静态类型可以预防一类 bug 干扰你的程序，准确地说，是预防对错误的参数类型应用某种操作而产生的 bug。如果你编写了一个如下的代码：
...
上述代码尝试将两个字符串相乘，当你在编程时静态类型就会捕获到该错误，而不是等到执行到该代码时。
...
类似于 Mypy 这样的检查工具可以在不运行代码的情况下利用声明的类型来静态地寻找类型错误。
...
为这些动态类型语言增加静态类型支持，反映了一种在软件工程师中广泛传播的理念——即静态类型的使用对于构建和维护大型软件系统来说是很重要的。对比于像 Java 这样的静态类型语言，为一种动态类型语言添加静态类型支持创造了一种名为[渐进式类型化](https://en.wikipedia.org/wiki/Gradual_typing)的编程方法，该方法在代码的某部分增添静态类型声明，而其他部分则略去。渐进式类型化可为一个小的、试验性的原型成长为一个大型的、稳定的、可维护的系统提供更加光滑的路径。

<!-- ## Static checking, dynamic checking, no checking -->
## 静态检查，动态检查，无检查

<!-- It’s useful to think about three kinds of automatic checking that a language can provide: -->
可将一门编程语言可提供的自动化检查分为如下三类：
...
毋庸置疑，静态地捕获 bug 比动态地捕获 bug 更好，动态地捕获 bug 又比完全不捕获 bug 更好。
...
在 Java 和其他许多编程语言中都有这样一个问题：原始数值类型的极端情况与我们所用的整数和实数具有不同的行为。这样导致的后果是一些本应被动态检查到的错误完全没有被检查到。以下是一些例子：
...
- 整数除法。`5/2` 并不会返回一个分数，而是返回一个截断了分数后的小数点的整数。这是一个我们期望产生动态错误的例子（因为分数不可以被表示成整数），而不是产生一个错误的结果。
...
- 整数溢出。`int` 和 `long` 这两种类型实际上是一个有限的整数集合，其有最大值和最小值。所以当我们进行了某种计算，然后结果过大或过小，以至于超出了集合的范围，那么在这种情况下会发生什么呢？计算会静默地溢出，然后从集合的有效范围中返回一个整数值，这通常不是正确的结果。
...
- 浮点类型中的特殊值。像 `double` 这样的浮点类型有一些不是实数的特殊值：`NaN`（表示 "Not a Number"），`POSITIVE_INFINITY`，`NEGATIVE_INFINITY`。所以当你对一个 double 类型变量执行了某种操作，并且你期望该操作会产生动态错误，比如除数为零或是求取了负数的平方根，那么你就会得到上述的特殊值。如果说你继续用得到的结果进行后续的计算，那么你最终会得到一个错误的答案。
