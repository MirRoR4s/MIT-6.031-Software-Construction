...
**操作**（**Operations**）就是一种接收输入然后产生输出的函数（数学上的）（有时也会改变输入本身）。同一操作可能有不同的语法，但无论它们的编写方式如何，都可将它们视为函数。以下是 Python 或 Java 中某操作的三种不同语法形式：
...
一些操作是可以重载的，在这样的情况下，同一操作被用于不同的类型。Java 的原始数字类型大量重载了 `+`，`-`，`*`，`/` 算术运算符 。（比如 int 和 double 都用 `*` 表示乘法运算）。方法也可被重载，并注意到大部分的编程语言都有一定程度的重载。

---

## 静态类型

Java 是一种**静态类型**的语言，所有变量的类型在编译时（即程序运行之前）就已知晓，所以编译器可以推断出所有表达式的类型。如果 `a`、`b` 都是 `int`，那么编译器就可以推断出 `a+b` 也是 `int`。在像 python 这样的**动态类型语言**中，这种检查被推迟到运行时进行。（即程序运行的时候）

静态类型（Static typing）是一种特殊的**静态检查**（static checking,），会在编译时就检查 bug 的存在。静态类型可预防由参数类型错误而产生的 bug。如以下代码尝试将两个字符串相乘：

```java
"5" * "6"
```

当还在编程时，静态类型就可以捕获到该错误，而无需等到执行代码时。
...
静态检查可以检测到和变量类型相关联的错误，但通常情况下无法发现和该类型相关的特定值的错误。静态类型保证了一个变量必定具有来自其类型的某个值，但我们只有在运行的时候才可以确切地知道这个值是什么，所以如果错误是由特定值引起的，比如除数为零或是索引越界，那么编译器将不会抛出一个关于它的静态错误。
...
arrays（数组）是一种定长的序列类型。以下代码演示了如何声明一个数组变量并对其赋值：
...
所以最好使用 `List`（列表） 类型，而非定长的数组类型。列表是一种变长的序列类型。以下代码演示了如何声明一个 `List` 类型的变量。
...
为什么代码左侧是 `List`，但右侧却是 `ArrayList` 呢？`List` 实际上是一个接口，这是一种无法直接构造的类型，但它指定了一个 `List` 必须提供的操作，比如 `get()`、`set()`、`size()` 等等。`ArrayList` 是一个类，这是一个具体的类型，提供了上述操作的具体实现。

`ArrayList` 并不是 `List` 的唯一实现，`LinkedList` 同样也实现了 `List`。当在声明变量或返回值的类型时，我们倾向于使用 List，因为这会使得代码更加的通用和灵活，而无需关心具体使用了哪种类型的列表。
...
为什么代码是 `List<Integer>`，而不是 `List<int>`？因为 Java 不允许这么编写代码。List 只知道如何处理对象类型，而不知道如何处理原始类型。每个原始类型都有一个等价的对象类型。比如和 int 等价的对象类型是 `Integer`，和 long 等价的对象类型是 `Long`，和 float 等价的对象类型是 `Float`，和 double 等价的对象类型是 `Double`。当我们用一种类型来参数化另一种类型时，Java 要求我们使用这些类型等价的对象类型。在其他情况下，Java 会自动进行 `int` 和 `Integer` 的转换，所以我们直接编写 `Integer i = 5` 而不会引发类型错误。
...
写下一个变量的类型，并记录下对它的假设。比如变量总引用一个整数。Java 实际上会在编译时检查该假设，并保证你的程序没有违反它。
...
- 和他人交流。使程序易于理解，以便他人可在未来修复、改进或采用你的代码。
...
> 阅读完毕后建议到[原文处](https://web.mit.edu/6.031/www/sp21/classes/01-static-checking/)进行练习。
